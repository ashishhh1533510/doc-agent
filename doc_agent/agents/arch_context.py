...
def _parse_context(text: str) -> dict:
    """Strip code fence and parse JSON, same pattern as _parse_verdict in reviewer.py."""
    cleaned = text.strip()
    ...
    try:
        ...
        raise ValueError(f"ArchitectureContextAgent returned unparseable JSON: {text[:200]}") from e
